...
class DefaultLayout(jp.QLayout):
    def __init__(self, view="hHh lpR fFf", **kwargs):
        ...
        scroller_list = jp.QList(a=scroller)

        a_classes = "m-2 p-2 text-lg text-blue-400 hover:text-blue-700"
        jp.A(a=scroller_list, text="Home", href="/", classes=a_classes)
        jp.Br(a=scroller_list)
        jp.A(a=scroller_list, text="Dictionary", href="/dictionary", classes=a_classes)
        jp.Br(a=scroller_list)
        jp.A(a=scroller_list, text="About", href="/about", classes=a_classes)
